...
train_csv = '../evaluation_setup/fold1_train.csv'
val_csv = '../evaluation_setup/fold1_evaluate.csv'
#aug_csv = 'evaluation_setup/fold1_train_a_2003.csv'

# please generate logmel features using ../extr_feat_2020_nodelta_scaled.py at first
...
experiments = 'fsfcnn_%d/' %ind if not l2bn else 'fsfcnn_l2bn_%d/' %ind
...
model = model_fcnn(num_classes, 
                   ...
                   num_filters=[48, 96, 512, 768], wd=0)
...
log_dir = "./logs/fsfcnn_%d" %ind if not l2bn else './logs/fsfcnn_l2bn_%d/' %ind
tensorboard = TensorBoard(log_dir=log_dir,histogram_freq=0,write_graph=False)
